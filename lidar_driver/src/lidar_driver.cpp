...
{		
	...
	if(buffer.front()[2] != 1)
	{
		no_sample_points = buffer.front()[3];	
		
		start_angle_byte = buffer.front()[5] << 8 | buffer.front()[4];
		stop_angle_byte = buffer.front()[7] << 8 | buffer.front()[6];
		...
		for(int i=1; i<no_sample_points*2-2; i+=2)
		{
			...
			diatance_byte = buffer.front()[10+i] << 8 | buffer.front()[10+i-1];
